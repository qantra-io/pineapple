...
module.exports = class Pineapple {
	...
	//validation methods
	_type(vo) {
		// Split the type string into an array of types to check against each value
		let types = vo.type.split('|');
		let isValid = false;
		types.forEach(type => {
			let methodName = `is${type.charAt(0).toUpperCase() + type.toLowerCase().substring(1)}`;
			let method = this[methodName];
			if (method)
				isValid = isValid || method(vo.propValue);
			else throw Error(`Unable to find method relevant with: ${type}`)
		});
		return isValid;
	}
	_oneOf(vo) {
